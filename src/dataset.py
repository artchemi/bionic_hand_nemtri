...
def gestures(emg, label, targets=[0, 1, 3, 6],
    ...
    if relax_shrink != None:
        ...
        if len(gestures[0]) >= relax_shrink > 0:
            gestures[0] = random.sample(gestures[0], relax_shrink)
        else:
            print("⚠️ Слишком мало данных в gestures[0], пропускаем random.sample()")
        # gestures[0] = random.sample(gestures[0], relax_shrink)
...
def train_test_split(gestures, split_size=0.25, rand_seed=config.SEED):
    """
    Purpose:
...
def gestures2dict(gestures, rand_seed=config.SEED) -> dict:
    """Преобразует жесты для сценария тестирования в словарь.

    Args:
        gestures (_type_): Жесты.

    Returns:
        dict: Подходящий тип данных для дообучения
    """
    person_gestures = {key:None for key in gestures}

    for _, (label, signals) in enumerate(gestures.items()):
        random.Random(rand_seed).shuffle(signals)

        person_gestures[label] = signals
